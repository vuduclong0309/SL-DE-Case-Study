...
# Dependecy set up
wait_upstream_orders = ExternalTaskSensor("wait for input order table to finish")
wait_upstream_products = ExternalTaskSensor("wait for input product table to finish")

# Task to run ods_order
ods_order = SHHOperator(
    task_id = 'ods_order',
    hook = some_hook to maybe AWS,
    command = 'some command to run the dml sql',
)

dim_products = SHHOperator(
    task_id = 'ods_order',
    hook = some_hook to maybe AWS,
    command = 'some command to run the dml sql',
)
...
# End task
item_segment_ads_item_features_td_end = DummyOperator(
    task_id='item_segment_ads_item_features_td_end',
    dag=dag
)
...
## DAG set up
start >> wait_upstream_orders >> ods_order >> end
start >> wait_upstream_products >> dim_products >> end
