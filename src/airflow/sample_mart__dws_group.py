...
marker_dependencies = {
    'sample_mart__dwd_group': {
        'marker_name': 'sample_mart__dwd_group',
...
# Dependecy set up
wait_upstream_dwd = ExternalTaskSensor(
    task_id='wait_upstream_dwd',
    soft_fail=False,
    retries=2,
    external_task_id='end',
    external_dag_id='sample_mart__dwd_group',
    dag=dag)

# Task to run dws
dws_product_item_sold_1d = SHHOperator(
    task_id = 'dws_product_item_sold_1d',
    hook = some_hook to maybe AWS,
    command = 'some command to run the dml sql',
)

dws_product_item_sold_7d = SHHOperator(
    task_id = 'dws_product_item_sold_1d',
    hook = some_hook to maybe AWS,
    command = 'some command to run the dml sql',
)
...
## DAG set up
start >> wait_upstream_dwd >> [dws table 1d list] >> [dws table nd list] >> email_success >> end
